...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height, bar_width, panel_height, panel_y, mouse, colors, game_state):
		...
		for y in range(game_map.height):
			for x in range(game_map.width):
				...
				if visible:
					...
					if wall:
						libtcod.console_put_char_ex(con, x, y, '#', libtcod.dark_grey, libtcod.black)
					else:
						libtcod.console_put_char_ex(con, x, y, '.', libtcod.dark_grey, libtcod.black)
	...
	if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY):
		if game_state == GameStates.SHOW_INVENTORY:
			inventory_title = 'Choose item to Use, or esc to cancel.\n'
			
		else:
			inventory_title = 'Choose item to Drop, or esc to cancel.\n'
